...
import { Calendar1Icon, CheckIcon, DeleteIcon, StarIcon } from 'lucide-react';
import { Kconverter } from '../../lib/Kconverter';

const AddShows = () => {
  const currency = import.meta.env.BITE_CURRENCY;
    ...
    <>
            ...
            <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={() => setSelectedMovie(movie.id)}>
              <div className='relative rounded-lg overflow-hidden'>
                ...
                <div className='text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute bottom-0 left-0'>
                  <p className='w-4 h-4 text-[#D63854] fill-[#D63854]'>
                    <StarIcon className='w-4 h-4 text-[#D63854] fill-[#D63854]' />{movie.vote_average.toFixed(1)}
                  </p>
                  <p className='text-gray-300 '>{Kconverter(movie.vote_count)} Votes</p>
                </div>
              </div>
              {selectedMovie === movie.id && (<div className='absolute top-2 right-2 flex items-center justify-center bg-[#D63854] h-6 w-6 rounded'>
      ...
      <div className='mt-6'>
          ...
          <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
            ...
            <button onClick={handleDateTimeAdd} className='bg-[#D63854]/80  text-white px-3 py-2 text-sm rounded-lg hover:bg-[#D63854] cursor-pointer'>Add Time</button>
          </div>
      </div>
              ...
              {Object.entries(dateTimeSelection).map(([date,times])=>(
                <li key={date}>
                  ...
                  <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                    {times.map((time)=>(
                      <div key={time} className='border border-[#D63854] px-2 py-1 flex items-center rounded '>
                        <span>{time}</span>
      ...
      <button className='bg-[#D63854] text-white px-8 py-2 mt-6 rounded hover:bg-[#D63854]/90 transition-all cursor-pointer'>Add Show</button>
    </>
  ) : <Loading />
}
